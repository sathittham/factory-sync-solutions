...
func TestHandlerGetEngagement(t *testing.T) {
	tests := []struct {
		name       string
		path       string
		mock       *mockGA4Client
	...
	}{
		{
			...
			mock: &mockGA4Client{
				respFunc: func(callIdx int, req *analyticsdata.RunReportRequest) (*analyticsdata.RunReportResponse, error) {
					return &analyticsdata.RunReportResponse{
						Rows: []*analyticsdata.Row{row([]string{"20260601"}, []string{"10", "40", "120"})},
					}, nil
				},
			},
			wantStatus: http.StatusOK,
		...
		{
			name:       "invalid range returns 400 VALIDATION_ERROR",
			path:       "/analytics/engagement?range=365d",
			mock:       &mockGA4Client{},
			wantStatus: http.StatusBadRequest,
			wantCode:   "VALIDATION_ERROR",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !ok {
				t.Fatalf("expected current object, got %v", data["current"])
			}
			if current["dau"] != float64(10) {
				t.Errorf("current.dau = %v, want 10", current["dau"])
			}
		})
	}
}
...
func TestHandlerGetSources(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			mock: &mockGA4Client{
				respFunc: func(callIdx int, req *analyticsdata.RunReportRequest) (*analyticsdata.RunReportResponse, error) {
					return &analyticsdata.RunReportResponse{
						Rows: []*analyticsdata.Row{row([]string{"google / organic"}, []string{"120"})},
					}, nil
				},
			},
			wantStatus: http.StatusOK,
		...
		{
			name:       "invalid range returns 400 VALIDATION_ERROR",
			path:       "/analytics/sources?range=365d",
			mock:       &mockGA4Client{},
			wantStatus: http.StatusBadRequest,
			wantCode:   "VALIDATION_ERROR",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !ok || len(sources) != 1 {
				t.Fatalf("expected 1 source row, got %v", data["sources"])
			}
		})
	}
}
...
func TestHandlerGetMeta(t *testing.T) {
	t.Run("success returns propertyID", func(t *testing.T) {
		svc := NewService(&mockGA4Client{}, time.Minute)
		svc.propertyID = "540943523"
		router := newTestRouter(svc)

		req := httptest.NewRequest(http.MethodGet, "/analytics/meta", nil)
		rec := httptest.NewRecorder()
		router.ServeHTTP(rec, req)
		...
		if data["propertyID"] != "540943523" {
			t.Errorf("propertyID = %v, want 540943523", data["propertyID"])
		}
	})
