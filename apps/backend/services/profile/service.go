...
// SetEventPublisher attaches a domain event publisher used for future service extraction.
...
func (s *Service) CreateProfile(ctx context.Context, uid, email, displayName, avatarURL string, req *CreateProfileRequest) (*Profile, error) {
	...
	s.publishEvent(ctx, uid, events.DomainEventProfileRegistered, "profile", uid, map[string]any{
		"uid":          uid,
		"companyRegId": req.CompanyRegID,
		"companyName":  req.CompanyName,
...
func (s *Service) CreateInvitedProfile(ctx context.Context, p *Profile) (*Profile, error) {
	...
	s.publishEvent(ctx, p.UID, events.DomainEventProfileRegistered, "profile", p.UID, map[string]any{
		"uid":             p.UID,
		"companyRegId":    p.CompanyRegID,
		"companyName":     p.CompanyName,
		"inviteAccepted":  true,
		"source":          "CreateInvitedProfile",
	}, map[string]any{
		"companyName": p.CompanyName,
		"source":      "CreateInvitedProfile",
	})
...
func (s *Service) UpdateProfile(ctx context.Context, uid string, req *UpdateProfileRequest) (*Profile, error) {
	...
	s.publishEvent(ctx, uid, events.DomainEventProfileUpdated, "profile", uid, map[string]any{
		"uid":          uid,
		"projectRegId": existing.CompanyRegID,
...
func (s *Service) publishEvent(ctx context.Context, actorUID, eventType, aggregateType, aggregateID string, payload map[string]any, metadata map[string]any) {
	if s.eventPub == nil {
		return
	}
	if err := s.eventPub.Publish(ctx, events.NewEvent(eventType, aggregateType, aggregateID, actorUID, payload, metadata)); err != nil {
		slog.Warn("failed to publish profile domain event", "eventType", eventType, "aggregateID", aggregateID, "error", err)
	}
}
