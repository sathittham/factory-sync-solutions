...
func (s *Service) SubmitQuiz(ctx context.Context, uid, contactEmail, contactName, companyName, quizID string, emailNotifications bool, answers []scoring.QuizAnswer) (*result.Assessment, error) {
	...
	s.publishEvent(ctx, uid, events.DomainEventQuizSubmitted, "quiz", assessment.ID, map[string]any{
	...
	s.publishEvent(ctx, uid, events.DomainEventResultReady, "assessment", assessment.ID, map[string]any{
		"assessmentId": assessment.ID,
		"uid":          uid,
		"quizId":       quizID,
		"overallScore": scoringResult.OverallScore,
...
func (s *Service) publishEvent(ctx context.Context, actorUID, eventType, aggregateType, aggregateID string, payload map[string]any, metadata map[string]any) {
	...
	if err := s.eventPub.Publish(ctx, events.NewEvent(eventType, aggregateType, aggregateID, actorUID, payload, metadata)); err != nil {
		slog.Warn("failed to publish quiz domain event", "eventType", eventType, "aggregateID", aggregateID, "error", err)
	}
}
