...
func TestSubmitQuiz_Success(t *testing.T) {
	...
	assessment, err := svc.SubmitQuiz(context.Background(), "u-1", "test@example.com", "Test", "TestCo", "shindan", true, answers)
	if err != nil {
...
func TestSubmitQuiz_IncompleteAnswers(t *testing.T) {
	...
	_, err := svc.SubmitQuiz(context.Background(), "u-1", "", "", "", "shindan", true, answers)
...
func TestSubmitQuiz_QuizNotFound(t *testing.T) {
	...
	answers := []scoring.QuizAnswer{{QuestionID: "q1", Value: 3}}
	_, err := svc.SubmitQuiz(context.Background(), "u-1", "", "", "", "nonexistent", true, answers)
