...
import { setAvailableQuizzes, setQuizId, resetQuiz } from "@/store/quizSlice";
...
export function DashboardPage() {
	...
	useEffect(() => {
		if (availableQuizzes.length === 0) {
			api.get<QuizListItem[]>("/quiz/quizzes")
			...
			<div className="container max-w-5xl py-6 sm:py-8 px-4 sm:px-6 space-y-6">
				...
				{Object.keys(quizGroups).length > 0 && (
					<div className="space-y-3">
						...
						<StaggerChildren className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" stagger={0.08}>
										...
										>
											<div className="flex items-start gap-4">
												...
												<div className="flex-1 min-w-0">
													...
													<Badge className={`text-[10px] px-2 py-0.5 ${diag.bg} ${diag.color} ${diag.border} border font-medium`}>
													...
													{quizAssessments.length > 1 && (
														<p className="text-[11px] text-muted-foreground mt-0.5">
															{t("quiz.totalAssessments").replace("{count}", String(quizAssessments.length))}
														</p>
													)}
												</div>
											</div>
							...
							{uncompletedQuizzes.map((q) => (
								...
								>
									<div className="flex items-center gap-4">
										<div className="h-11 w-11 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 flex items-center justify-center flex-shrink-0 group-hover:bg-emerald-100/70 dark:group-hover:bg-emerald-900/30 transition-colors">
					...
					<ScaleIn>
						<div className="text-center py-12">
							...
							<p className="text-sm text-muted-foreground mb-6">
								{locale === "th" ? "เริ่มทำแบบประเมินเพื่อตรวจสุขภาพโรงงานของคุณ" : "Start an assessment to check your factory health"}
							</p>
						</div>
					</ScaleIn>
				)}
