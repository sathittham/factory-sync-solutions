...
import type { QuizListItem, QuizDimension } from "@/store/quizSlice";
...
function QuizResultDetail({ assessment, dimLookup, locale, t, isDark }: Readonly<{
	...
	const dimName = useCallback((s: DimensionScore) => {
...
export function ResultPage() {
	...
	useEffect(() => {
		if (activeQuizId && !dimCache[activeQuizId]) {
			api
				.get<{ dimensions: QuizDimension[]; questions: unknown[] }>(`/quiz/questions?quizId=${activeQuizId}`)
				.then((data) => {
	...
	return (
		<div className="min-h-[calc(100vh-3.5rem)]">
			<div className="container max-w-5xl py-6 sm:py-8 space-y-5" data-testid="result-summary">
				{allQuizIds.length > 0 && (
					<Tabs
						value={activeQuizId ?? allQuizIds[0]}
					...
					>
							...
							{allQuizIds.map((qid) => {
								...
								return (
									<TabsTrigger
										key={qid}
										value={qid}
										className={hasResult ? "" : "opacity-50"}
									>
										{getQuizName(qid)}
									</TabsTrigger>
								);
						...
						{allQuizIds.map((qid) => {
							...
							const selectedAssessment = assessment && (assessment.quizId || "shindan") === qid
								? assessment
								: quizAssessments[0] ?? null;
							const dims = dimCache[qid] ?? [];
							const lookup = Object.fromEntries(dims.map((d) => [d.id, { th: d.nameTh, en: d.nameEn }]));
