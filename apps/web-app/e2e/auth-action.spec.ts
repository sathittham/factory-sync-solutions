...
test.describe('Auth Action Page (/auth/action)', () => {
  test('shows error card with no oobCode', { tag: '@regression' }, async ({ page }) => {
  ...
  test(
    'shows password form with valid mode and oobCode',
    { tag: '@regression' },
    async ({ page }) => {
  ...
  test(
    'unauthenticated access is allowed (no redirect)',
    { tag: '@regression' },
    async ({ page }) => {
