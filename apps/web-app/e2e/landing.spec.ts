...
test.describe('Landing Page', () => {
  ...
  test('renders hero section with CTA button', { tag: '@regression' }, async ({ page }) => {
    const heroCta = page.getByTestId('hero-cta-btn');
    await expect(heroCta).toBeVisible();
  });

  test('renders bottom CTA section', { tag: '@regression' }, async ({ page }) => {
    const bottomCta = page.getByTestId('bottom-cta-btn');
    await expect(bottomCta).toBeVisible();
  });

  test(
    'hero CTA opens sign-in dialog with Google button',
    { tag: '@regression' },
    async ({ page }) => {
  ...
  test('renders LINE contact button', { tag: '@regression' }, async ({ page }) => {
    const lineBtn = page.getByTestId('line-cta-btn');
    await expect(lineBtn).toBeVisible();
  });

  test('has footer', { tag: '@regression' }, async ({ page }) => {
