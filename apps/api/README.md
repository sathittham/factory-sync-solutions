# Factory Health Check — API

Go REST API powering the Factory Health Check assessment platform. Handles authentication, profile management, quiz submission with rubric-based scoring, result storage, admin operations, and notifications.
...
├── config/
│   └── questions.json     # Quiz questions & dimension definitions
├── middleware/
...
└── services/
    ...
    ├── dbd/
    │   ├── handler.go     # DBD (Department of Business Development) lookup
    │   ├── service.go
    │   └── models.go
    ├── notification/
