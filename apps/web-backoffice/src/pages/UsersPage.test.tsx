...
const userAdmin = { ...user, uid: 'uid-user-2', email: 'admin@example.com', role: 'admin' };
...
// Blank role + blank displayName/email — exercises the roleBadge fallback and
// getInitials' "no source at all" branch.
const userBlank = { ...user, uid: 'uid-user-4', email: '', displayName: '', role: '' };
...
describe('UsersPage', () => {
  ...
  it('shows an error message when deleting a user fails', async () => {
    mockedBackofficeApi.listUsers.mockResolvedValue([user]);
    mockedBackofficeApi.deleteUser.mockRejectedValue(new Error('boom'));
  ...
  it('closes the delete confirmation via Cancel without deleting', async () => {
    ...
    expect(mockedBackofficeApi.deleteUser).not.toHaveBeenCalled();
  });
  ...
  it('closes the user profile dialog on Escape', async () => {
    mockedBackofficeApi.listUsers.mockResolvedValue([user]);
  ...
  it('closes the audit activity dialog for a user', async () => {
    mockedBackofficeApi.listUsers.mockResolvedValue([user]);
  ...
  it('opens the user profile dialog by clicking the row', async () => {
    mockedBackofficeApi.listUsers.mockResolvedValue([user]);

    renderPage();
    await screen.findByText('Jane User');

    fireEvent.click(screen.getByText('Jane User').closest('tr') as HTMLElement);

    expect(await screen.findByText('User Profile')).toBeInTheDocument();
  });

  it('opens the user profile dialog by pressing Enter on a focused row', async () => {
    mockedBackofficeApi.listUsers.mockResolvedValue([user]);

    renderPage();
    await screen.findByText('Jane User');

    const row = screen.getByText('Jane User').closest('tr') as HTMLElement;
    fireEvent.keyDown(row, { key: 'Enter' });

    expect(await screen.findByText('User Profile')).toBeInTheDocument();
  });

  it('does not open the profile dialog for a non-Enter key on a focused row', async () => {
    mockedBackofficeApi.listUsers.mockResolvedValue([user]);
  ...
  it('renders the correct role badge for admin, system_admin, and blank roles', async () => {
    mockedBackofficeApi.listUsers.mockResolvedValue([userAdmin, userSystemAdmin, userBlank]);
    ...
    // userBlank has no displayName/email — getInitials falls back to '?'.
