...
type profileGetter interface {
	GetProfileData(ctx interface{}, uid string) (contactEmail, contactName, companyName string, emailNotifications bool, err error)
}
...
func (h *Handler) SubmitQuiz(w http.ResponseWriter, r *http.Request) {
	...
	if h.profileSvc != nil {
		var err error
		contactEmail, contactName, companyName, emailNotifications, err = h.profileSvc.GetProfileData(r.Context(), uid)
		if err != nil {
			slog.Warn("could not fetch profile for notification", "uid", uid, "error", err.Error())
		}
	}

	assessment, err := h.service.SubmitQuiz(r.Context(), uid, contactEmail, contactName, companyName, req.QuizID, emailNotifications, req.Answers)
	if err != nil {
		handleError(w, r, err)
		return
	}
