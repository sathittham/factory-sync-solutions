...
func buildResultEmailHTML(contactName, companyName string, overallScore float64, diagnosis string, scores []scoring.DimensionScore, strengths, weaknesses []string) string {
	...
	fmt.Fprintf(&sb, `<p>Dear %s,</p>`, html.EscapeString(contactName))
	fmt.Fprintf(&sb, `<p>Here are the assessment results for <strong>%s</strong>.</p>`, html.EscapeString(companyName))

	// Overall score
	fmt.Fprintf(&sb, `<div style="background:#f0f7ff;padding:20px;border-radius:8px;margin:20px 0;text-align:center;">
	...
	for _, ds := range scores {
		fmt.Fprintf(&sb, `<tr><td style="padding:8px;border-bottom:1px solid #e5e7eb;">%s</td><td style="text-align:right;padding:8px;border-bottom:1px solid #e5e7eb;">%.2f</td></tr>`, html.EscapeString(ds.DimensionName), ds.Score)
	}
	...
	if len(strengths) > 0 {
		...
		for _, s := range strengths {
			fmt.Fprintf(&sb, `<li>%s</li>`, html.EscapeString(s))
		}
		sb.WriteString(`</ul>`)
	}
	...
	if len(weaknesses) > 0 {
		...
		for _, w := range weaknesses {
			fmt.Fprintf(&sb, `<li>%s</li>`, html.EscapeString(w))
		}
		sb.WriteString(`</ul>`)
	}

	fmt.Fprintf(&sb, `<hr style="border:none;border-top:1px solid #e5e7eb;margin:30px 0;">
